...
Podemos encontrar varias funciones: `slice()`, `slice_head()`, `slice_tail()`, `slice_min()`, `slice_max()`, `slice_sample()`.
...
Tiene dos argumentos muy útiles *.before* y *.after*.
...
La sintaxis siempre es `nuevo_nombre = viejo_nombre`. `rename_with()` renombra columnas utilizando una función. 
...
`add_count()` y `add_tally()` son equivalentes a `count()` y `tally()` pero utilizan `mutate()` en lugar de `summarise()` para añadir una nueva columna con recuentos por grupos.
...
`group_by` no cambia los datos pero, si observa detenidamente la salida, se dará cuenta de que la salida indica que está «agrupada por» (`Groups: species, island [5]`). Esto significa que las operaciones posteriores funcionarán ahora por grupo. Por lo cual es recomendable usar `dplyr::ungroup` cuando sea necesario.
...
La operación de agrupamiento más importante. Reduce el data frame para que tenga una sola fila para cada grupo. `summarise()` y `summary()` son sinónimos.

Funciones útiles: centrales (`mean`, `median`), dispersión (`sd`, `IQR`, `mad`), rango (`min`, `max`), posición (`first`, `last`, `nth`), recuento (`n`, `n_distinto`), lógicas (`any`, `all`).
...
Hay varias funciones: `read_csv`, `read_tsv`, `read_csv2`, `read_delim`.
...
Hay varias funciones: `read_excel`, `read_xls`, `read_xlsx`.
